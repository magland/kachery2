...
export const objectExists = async (bucket: Bucket, key: string): Promise<{exists: boolean, size: number | undefined}> => {
    return new Promise<{exists: boolean, size: number | undefined}>((resolve, reject) => {
        const s3 = getS3Client(bucket)
        s3.headObject({
            Bucket: bucketNameFromUri(bucket.uri),
            Key: key

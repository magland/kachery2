...
export const initiateUpload = async (a: {
    ...
    if (a.size > maxSizeForZone(a.zone)) {
        throw Error(`File size exceeds maximum for zone: ${a.size} > ${maxSizeForZone(a.zone)}`)
    }
    if (a.hashAlg !== 'sha1') {
...
const signedDownloadUrlCache = new Map<string, {
    url: string,
    expires: number,
    size: number,
    bucketUri: string,
...
const recordUpload = async (a: {
    ...
    const collection = client.db(dbName).collection(collectionNames.uploadRecords);
...
const recordDownload = async (a: {
    ...
    const client = await getMongoClient();
    const collection = client.db(dbName).collection(collectionNames.downloadRecords);
    await collection.insertOne({
        stage: a.stage,
        timestamp: a.timestamp,
        zone: a.zone,
...
const pendingUploadCache = new Map<string, {
    expires: number
}>()
